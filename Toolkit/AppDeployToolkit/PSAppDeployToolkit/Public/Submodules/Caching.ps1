...
function Copy-ContentToCache
{
    ...
    This can be done using [Remove-File -Path "(Get-ADTSession).GetConfig().Toolkit_Options.Toolkit_CachePath\$installName" -Recurse -ContinueOnError $true]
    ...
    Param (
        ...
        [String]$Path = "$((Get-ADTSession).GetConfig().Toolkit_Options.Toolkit_CachePath)\$installName"
    )
