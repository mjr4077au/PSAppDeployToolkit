...
function Execute-MSI
{
    ...
    .EXAMPLE
    # Install an MSI and store the result of the execution into a variable by using the -PassThru option
    ...
    Process {
        ...
        Else {
            ...
            If (-not (Test-Path -LiteralPath (Get-ADTSession).GetConfig().MSI_Options.MSI_LogPath -PathType 'Container' -ErrorAction 'SilentlyContinue')) {
                $null = New-Item -Path (Get-ADTSession).GetConfig().MSI_Options.MSI_LogPath -ItemType 'Directory' -ErrorAction 'SilentlyContinue'
            }
            ## Build the log file path
            [String]$logPath = Join-Path -Path (Get-ADTSession).GetConfig().MSI_Options.MSI_LogPath -ChildPath $LogName
        }

        ## Set the installation Parameters
        If ($deployModeSilent) {
            $msiInstallDefaultParams = (Get-ADTSession).GetConfig().MSI_Options.MSI_SilentParams
            $msiUninstallDefaultParams = (Get-ADTSession).GetConfig().MSI_Options.MSI_SilentParams
        }
        Else {
            $msiInstallDefaultParams = (Get-ADTSession).GetConfig().MSI_Options.MSI_InstallParams
            $msiUninstallDefaultParams = (Get-ADTSession).GetConfig().MSI_Options.MSI_UninstallParams
        }
        ...
        Else {
            $argsMSI = "$argsMSI $((Get-ADTSession).GetConfig().MSI_Options.MSI_LoggingOptions) $msiLogFile"
        }
...
function Remove-MSIApplications
{
    ...
    .OUTPUTS
    PSObject. Returns an object with the following properties:
    - ExitCode
    - StdOut
    - StdErr

    .EXAMPLE
    # Remove all versions of software that match the name "Adobe Flash"
    Remove-MSIApplications -Name 'Adobe Flash'

    .EXAMPLE
    # Remove all versions of software that match the name "Adobe"
    Remove-MSIApplications -Name 'Adobe'

    .EXAMPLE
    # Remove all versions of software that match the name "Java 8 Update" where the software is 32-bits and the publisher is "Oracle Corporation".
    ...
    .EXAMPLE
    # Remove all versions of software that match the name "Java 8 Update"; however, it does not uninstall 64-bit versions of the software, Update 45 of the software, or any Update that starts with 4.
